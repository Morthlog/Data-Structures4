...
public class LinearProbingHashStDouble2<Key, V> {
// must be a power of 2
   ...
   private Node2<Key,V>[] indexes;    // the values
   int primeSize;

   /**
    * Initializes an empty symbol table.
    */
   public LinearProbingHashStDouble2() 
   {
       this(INIT_CAPACITY);
   }
    ...
    */
   public LinearProbingHashStDouble2(int capacity)
   {
...
//	   return (key.hashCode() & 0x7fffffff)%(m-1);
    ...
    */
   public void put(Key key, Node2<Key,V> val) 
   {
       if (key == null) 
...
//    	   System.out.println("put "+" key= "+key+" i="+ i +" k="+k);
    ...
    * @throws IllegalArgumentException if {@code key} is {@code null}
    */
   public Node2<Key,V> get(Key key) 
   {
       if (key == null) 
       ...
       {   
    	...
    	//   System.out.println("get "+" key= "+key+" i="+ i +" k="+k);     
    	   if (keys[i].equals(key) &&  indexes[i] != null)
               return indexes[i];
    	   if(i==initial & temp==2)
    	   {
   ...
   public void delete(Key key) 
   {
       ...
       { 
           ...
           if (key.equals(keys[i]) && indexes[i] != null )
           {
